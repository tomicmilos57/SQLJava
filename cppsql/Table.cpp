...
Table::Table(
    ...
    std::vector<std::string> const
        &columns_name) { // Throw exception if columns have the same name
  this->name = name;
  this->columns_name =
      columns_name; // Throw exception if columns are empty in query_parser
  std::for_each(this->columns_name.begin(), this->columns_name.end(),
                [this](std::string s) {
                  this->table.push_back(std::vector<std::string>());
                });
}
Table::Table(Table const &t1, Table const &t2, std::string abbr1,
             std::string abbr2, Where const &viewWhere) { // for inner join
  ...
  for (int i = 0; i < t1.size; i++) {
    ...
    for (int k = 0; k < t1.table.size(); k++)
      row1.push_back(t1.table[k][i]);
    for (int j = 0; j < t2.size; j++) {
      std::vector<std::string> row2;
      for (int k = 0; k < t2.table.size(); k++)
        row2.push_back(t2.table[k][j]);
      std::vector<std::string> temp;
      temp.insert(temp.end(), row1.begin(), row1.end());
      temp.insert(temp.end(), row2.begin(), row2.end());
      if (viewWhere.conditionTrue(temp, columns_name))
        this->insert_into(this->columns_name, temp);
    }
  }
}
void Table::save_sql(std::ofstream &os) const {
  ...
  insert_template << "INSERT INTO " << this->name << " " << columns.str()
                  << " VALUES (";
...
std::string Table::print_Table() const {
...
void Table::print_columns(std::ostream &os,
                          std::vector<std::string> const &columns_name,
                          std::string const &name) {
  os << /*"___________________________________________\n" +*/ name << std::endl;
  std::for_each(columns_name.begin(), columns_name.end(),
                [&os](std::string s) { os << std::setw(15) << s << '\t'; });
  os << '\n';
}
std::string Table::select(std::vector<std::string> const &columnsSelect,
                          Where const &where) const {
  std::vector<std::string> columnsToSelect = columnsSelect;
  if (columnsSelect.size() == 1 && columnsSelect[0] == "*")
    columnsToSelect = columns_name;
  std::stringstream print;
    ...
    if (where.conditionTrue(table, i, columns_name)) {
      for (auto str : columnsToSelect) {
        ...
        for (; j < columns_name.size(); j++) { // finding the index of a column
          if (columns_name[j] == str)
            break;
        }
        if (j == columns_name.size())
          throw std::string("Cannot find the column " + str);
        print << std::setw(15) << (table[j])[i]
              << "\t"; // this should append to a string and print a string at
                       // the end
      }
      print << "\n";
    }
...
std::string Table::innerjoin(Table const &Table1, Table const &Table2,
                             std::vector<std::string> const &columns,
                             std::string abbreviation1,
                             std::string abbreviation2, Where &viewWhere,
                             Where &where) {
  Table view(Table1, Table2, abbreviation1, abbreviation2, viewWhere);
  return view.select(columns, where);
}
void Table::insert_into(std::vector<std::string> const &where_to_insert,
                        std::vector<std::string> const &what_to_insert) {
  if (where_to_insert.size() != what_to_insert.size())
    throw "Error"; // throw this in parser
...
void Table::_delete(Where const &where) {
  for (int i = 0; i < size; i++) {
    if (where.conditionTrue(table, i, columns_name)) {
      for (int j = 0; j < columns_name.size();
           j++) { // finding the index of a column
