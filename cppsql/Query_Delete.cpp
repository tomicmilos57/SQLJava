...
Query_Delete::Query_Delete(std::vector<std::string> tokens) : Query(tokens) {
  if (!(lower(tokens.at(0)) == "delete" && lower(tokens.at(1)) == "from"))
    throw std::string("Wrong Fromat");
  tableName = tokens.at(2);
  int i = 3;
  where = Where::read_where(i, tokens, 1);
}
std::string Query_Delete::execute(std::vector<Table> *tables) {
  ...
  if (tablePointer == nullptr)
    throw "There isn't a table with that name ";
  tablePointer->_delete(where);
  return std::string("Deleted Successfully");
}
